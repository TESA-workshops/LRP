...
ggplot(D) + 
  ...
  geom_hline(yintercept=SSB0, linetype="dashed", color = "green") +                      #SSB0 based on phi0 from first 5 historical years
  ...
  geom_path(mapping=aes(y=SSBmsy_SSB0*dSSB0a,x=Year),color="red",linetype=2) +           #full dynamic SSBmsy, assume ratio of dynamic SSBmsy/SSB0 = same ratio as equilibrium SSBmsy/SSB0
